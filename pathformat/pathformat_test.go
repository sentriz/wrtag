...
func TestPathFormatMultiDisc(t *testing.T) {
	...
	t.Run("disc number in path", func(t *testing.T) {
		t.Parallel()
		var pf pathformat.Format
		require.NoError(t, pf.Parse(`/music/{{ .Release.Title }}/{{ .DiscNum }}-{{ pad0 2 .Track.Position }} {{ .Track.Title }}{{ .Ext }}`))

		// Disc 1, Track 1
		path, err := pf.Execute(release, 0, ".flac")
		require.NoError(t, err)
		assert.Equal(t, `/music/Reise, Reise/1-01 Reise, Reise.flac`, path)

		// Disc 1, Track 2
		path, err = pf.Execute(release, 1, ".flac")
		require.NoError(t, err)
		assert.Equal(t, `/music/Reise, Reise/1-02 Mein Teil.flac`, path)

		// Disc 2, Track 1
		path, err = pf.Execute(release, 2, ".flac")
		require.NoError(t, err)
		assert.Equal(t, `/music/Reise, Reise/2-01 Dalai Lama.flac`, path)

		// Disc 2, Track 2
		path, err = pf.Execute(release, 3, ".flac")
		require.NoError(t, err)
		assert.Equal(t, `/music/Reise, Reise/2-02 Keine Lust.flac`, path)
	})
	...
	t.Run("disc title in path", func(t *testing.T) {
		...
		require.NoError(t, pf.Parse(`/music/{{ .Release.Title }}/{{ if .DiscTitle }}{{ .DiscTitle | safepath }}-{{ end }}{{ .Track.Title }}{{ .Ext }}`))
