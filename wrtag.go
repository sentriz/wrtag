...
func ProcessDir(
	...
	slog.InfoContext(ctx, "scanning directory", "path", srcDir)
	...
	// calculate new paths
	slog.InfoContext(ctx, "calculating destination paths", "count", len(pathTags))
	...
	// move/copy and tag
	slog.InfoContext(ctx, "processing files", "operation", op.Name(), "count", len(pathTags))
	...
	if err := processCover(ctx, cfg, op, dc, release, albumRoot, cover); err != nil {
	...
	for kf := range cfg.KeepFiles {
		if err := op.ProcessPath(ctx, dc, filepath.Join(srcDir, kf), filepath.Join(albumRoot, kf)); err != nil && !errors.Is(err, os.ErrNotExist) {
			return nil, fmt.Errorf("process keep file %q: %w", kf, err)
		}
	}

	if err := trimDestDir(ctx, dc, albumRoot, op.CanModifyDest()); err != nil {
...
func CommonDirPrefix(paths []string) string {
	...
	// Round to nearest complete directory
	...
	if !allMatch {
		// Prefix is partial, go to parent directory
