...
- `.TrackNum` - The track number (integer, starting at 1, sequential across all discs)
...
Due to the way wrtag represents releases, Releases with multiple discs need to be sorted into the same directory. (This is also how tools like [gonic](https://github.com/sentriz/gonic) expect the releases to be organized, [see #177](https://github.com/sentriz/wrtag/pull/177#issuecomment-3732860209)).

```
/music/{{ artists .Release.Artists | sort | join "; " | safepath }}/{{ .Release.Title | safepath }}{{ if gt .TotalDiscs 1 }}/{{ pad0 2 .DiscNum }}{{ end }}-{{ pad0 2 .Track.Position }} {{ .Track.Title | safepath }}{{ .Ext }}
```

Result for a 2-disc album:
```
/music/Artist/Album/01-01 Track Name.flac
/music/Artist/Album/01-02 Another Track.flac
/music/Artist/Album/02-01 First on Disc 2.flac
```

### Multi-disc with disc titles

If discs have titles (e.g., "Studio" and "Live"), you can include them in the path, keeping in mind the restriction that multiple discs from the same release must live in the same directory:

```
/music/{{ artists .Release.Artists | sort | join "; " | safepath }}/{{ .Release.Title | safepath }}/{{ if gt .TotalDiscs 1 }}{{ pad0 2 .DiscNum }}{{ if .DiscTitle }}/({{ .DiscTitle | safepath }}){{ end }}-{{ end }}{{ pad0 2 .Track.Position }} {{ .Track.Title | safepath }}{{ .Ext }}
```

Result:
```
/music/Artist/Album/01(Studio Sessions)-01 Track.flac
/music/Artist/Album/02(Live in London)-01 Track.flac
```
