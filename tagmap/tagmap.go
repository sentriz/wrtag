...
func ApplyConfig(
	...
	for _, k := range defaultCopyConfig {
		dest.Set(k, source.Values(k)...)
	}
	for _, k := range conf.Copy {
		dest.Set(k, source.Values(k)...)
	}
...
// defaultCopyConfig is set of tags which are copied over from the source file without being modified.
var defaultCopyConfig = []string{
